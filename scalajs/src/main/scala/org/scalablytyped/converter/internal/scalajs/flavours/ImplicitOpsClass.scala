  ...
  def apply(
    ...
    val sugarMembers: IArray[MethodTree] =
      applicableMembers.flatMap {
        case Prop(Prop.Variant(ParamTree(propName, _, _, _, _, _), _), _, variants, _) =>
          val variantsForProp: Map[Name, Prop.VariantLike] = {
            ...
            val (renamedUndefined, definedAlternatives, Empty) = variants.partitionCollect2(
              { case Prop.Undefined => Name(s"without${propName.unescaped.capitalize}") -> Prop.Undefined },
              { case v: Prop.Variant => v },
            )

            val renamedDefined =
              if (definedAlternatives.length === 1)
                Map(Name(s"with${propName.unescaped.capitalize}") -> definedAlternatives(0))
              else
            ...
            renamedDefined ++ renamedUndefined.toMap
          }
          ...
          val iterable = variantsForProp map {
            ...
            case (name, Prop.Variant(_param, asExpr)) =>
