...
/**
  ...
  */
final class GenCompanions(memberToProp: MemberToProp, findProps: FindProps, genImplicitSyntax: Boolean)
    ...
    props match {
      ...
      case props =>
        val (optionals, inLiterals, Empty) = props.partitionCollect2(
          { case Prop(Prop.Variant(_, Right(f)), _, _, _)  => f },
          { case Prop(Prop.Variant(_, Left(str)), _, _, _) => str },
        )
